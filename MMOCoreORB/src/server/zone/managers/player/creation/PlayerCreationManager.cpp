...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
					...
					try {
						...
						if (res != nullptr && res->next()) {
							...
							if (timeVal.miliDifference() < 1000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per second. Repeat attempts prior to 1 second elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
	...
	box->setPromptText("Welcome to SWGVision");
